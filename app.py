...
"""
    ...
    def manage_customers(self):
        ...
        with st.form("add_customer"):
            ...
            customer_data = {
                ...
                "phone": st.text_input("Phone Number", value=""),  # Set default empty string value
                "location": st.text_area("Location", value=""),  # Set default empty string value
                "signup_date": st.date_input("Signup Date", datetime.today().date()),  # Default to today's date
                ...
                "total_orders": st.number_input("Total Orders", min_value=0, value=0),  # Default to 0
            ...
            if st.form_submit_button("Create Customer"):
                if not customer_data["email"] or not customer_data["phone"]:
                    ...
                    # Ensure phone and location are empty strings instead of None
        ...
        # Display all customers with update and delete functionality
        ...
        for customer in customers:
            ...
            with st.expander(f"Actions for Customer ID {customer[0]}"):
                ...
                with st.form(f"update_customer_{customer[0]}"):
                    ...
                    # Ensure proper date format for signup_date
                    try:
                        updated_signup_date = datetime.strptime(customer[5], '%Y-%m-%d').date()
                    except Exception as e:
                        updated_signup_date = datetime.today().date()  # Default to today's date in case of error

                    updated_signup_date = st.date_input("Update Signup Date", updated_signup_date)

                    # Handle 'is_premium' field, ensuring it doesn't cause issues
                    ...
                    # Handling total_orders properly
                    ...
                    # Handling average_rating, ensure it's a float value
                    ...
                    # Validate updated phone and location fields
    ...
    def manage_restaurants(self): 
        ...
        # Display all restaurants with update and delete functionality
        ...
        for restaurant in restaurants:
            ...
            with st.expander(f"Actions for Restaurant ID {restaurant[0]}"):
                ...
                with st.form(f"update_restaurant_{restaurant[0]}"):
                    ...
                    # Ensure that average_delivery_time is an integer (convert if necessary)
                    try:
                    ...
                    # Ensure rating is an integer or float, and handle invalid values
                    try:
                        rating_value = float(restaurant[7]) if restaurant[7] is not None else 3.0  # Default to 3 if None
                        updated_rating = st.slider("Update Rating", 1, 5, int(rating_value))
                    except ValueError:
                        st.error("Invalid rating value.")
                        updated_rating = 3  # Default to 3 if there's an issue with the rating
    ...
    def manage_orders(self):
        ...
        # Display all orders with update and delete functionality
        ...
        for order in orders:
            ...
            with st.expander(f"Actions for Order ID {order[0]}"):
                ...
                with st.form(f"update_order_{order[0]}"):
                    ...
                    # Check if total_amount is numeric before passing to number_input
                    try:
                        total_amount_value = float(order[7])
                    except ValueError:
                        total_amount_value = 0.0  # If conversion fails, set default value
    ...
    def manage_deliveries(self): 
        ...
        # Display All Deliveries with Update/Delete Options
        ...
        for delivery in deliveries:
            with st.expander(f"Delivery ID: {delivery[0]} (Order ID: {delivery[1]}, Delivery Person ID: {delivery[2]})"):
                ...
                # Convert timedelta to total minutes for display
                if isinstance(delivery[5], timedelta):
                    delivery_time_minutes = delivery[5].total_seconds() / 60  # Convert timedelta to minutes
                else:
                    delivery_time_minutes = delivery[5]  # Assuming it's already in minutes if not timedelta
                ...
                with st.form(f"update_delivery_{delivery[0]}"):
                    ...
                    # Ensure that the correct type is passed for delivery_time
                    if isinstance(delivery[5], timedelta):
                    ...
                    # Ensure the correct type is passed for estimated_time
                    ...
                    # Update the list of vehicle types with 'Scooter' and other possible values
                    vehicle_types = ["Bike", "Car", "Van", "Bicycle", "Scooter", "Other"]
                    # Default to "Other" if the delivery vehicle type is not in the list
    ...
    def manage_delivery_persons(self):
        ...
        # View and Manage Existing Delivery Personnel
    ...
    def generate_fake_data(self):
        try:
            ...
            self.db_connection.rollback()  # Rollback any changes made before the error
