...
"""
    ...
    def show_homepage(self):
        ...
        st.image("D:\\GUVI-PROJECTS\\01-DS_ZOMATO_DATA_INSIGHTS\\zomato-app\\Scripts\\ezgif-6edec03c6ba51.gif")
    ...
    def manage_orders(self): 
        ...
        # Fetch customers and restaurants
        self.cursor.execute("SELECT customer_id, name FROM Customers")
        customers = self.cursor.fetchall()
        customer_options = [f"{c[0]} - {c[1]}" for c in customers]

        self.cursor.execute("SELECT restaurant_id, name FROM Restaurants")
        restaurants = self.cursor.fetchall()
        restaurant_options = [f"{r[0]} - {r[1]}" for r in restaurants]

        if not customer_options or not restaurant_options:
            st.warning("Please ensure customers and restaurants exist before adding an order.")
            return
        ...
        with st.form("add_order_form"):
            ...
            total_amount = st.number_input("Total Amount (₹)", min_value=0.0, step=10.0, format="%.2f")
            payment_mode = st.selectbox("Payment Mode", ["Credit Card", "Cash", "UPI"])
            discount_applied = st.number_input("Discount (₹)", min_value=0.0, step=5.0, format="%.2f")
            customer_feedback_rating = st.slider("Customer Feedback Rating", 1, 5, value=3)

            if st.form_submit_button("Add Order"):
                try:
                    customer_id = int(selected_customer.split(" - ")[0])
                    restaurant_id = int(selected_restaurant.split(" - ")[0])
                    self.cursor.execute("""
                        INSERT INTO Orders (
                            customer_id, restaurant_id, order_date, delivery_time, 
                            status, total_amount, payment_mode, discount_applied, customer_feedback_rating
                        )
                        VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s)
                    """, (
                        customer_id, restaurant_id, order_date, delivery_time, status,
                        total_amount, payment_mode, discount_applied, customer_feedback_rating
                    ))
                    self.db_connection.commit()
        ...
        try:
            ...
            for order in orders:
                order_id = order[0]
                order_payment_mode = order[7] if order[7] else "N/A"

                st.write(f"ID: {order_id}, Payment Mode: {order_payment_mode}")

                with st.expander(f"Actions for Order ID {order_id}"):
                    with st.form(f"update_order_{order_id}"):
                        updated_status = st.selectbox("Update Status", 
                            ["Pending", "Delivered", "Cancelled"], 
                        )
                        updated_total = st.number_input("Update Total Amount", 
                            min_value=0.0, value=float(order[6]) if order[6] is not None else 0.0, format="%.2f")
                        ...
                        if st.form_submit_button("Update Order"):
                            try:
                                self.cursor.execute("""
                                    ...
                                    WHERE order_id = %s
                                """, (updated_status, updated_total, order_id))
                                self.db_connection.commit()
                                st.success("Order updated successfully!")
                            except Exception as e:
                                st.error(f"Error updating order: {str(e)}")

                    if st.button("Delete Order", key=f"delete_order_{order_id}"):
                        try:
    ...
    def generate_fake_data(self):
        try:
            ...
            for _ in range(rd.randint(1, 10)):
                ...
                self.cursor.execute("""
                    INSERT INTO Orders (customer_id, restaurant_id, delivery_person_id, total_amount, distance, delivery_fee,
                                        payment_mode, customer_feedback_rating)
                    VALUES (%s, %s, %s, %s, %s, %s, %s, %s)
                """, (customer_id, restaurant_id, delivery_person_id, total_amount, distance, delivery_fee,
                    payment_mode, customer_feedback_rating))
                order_id = self.cursor.lastrowid

                delivery_time_minutes = rd.randint(20, 90)
                ...
                self.cursor.execute("""
                    INSERT INTO Deliveries (order_id, delivery_person_id, delivery_status, distance, delivery_time,
                                            estimated_time, delivery_fee, vehicle_type, delivery_date)
                    VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s)
                """, (order_id, delivery_person_id, order_status, distance, delivery_time_minutes,
                    estimated_time, delivery_fee, vehicle_type, delivery_date))
